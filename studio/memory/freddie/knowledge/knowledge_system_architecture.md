...
- **Usage Tracking (complete):** `libs/ai-usage.ts` auto-logs every Gemini call to `vault/studio/usage/YYYY-MM.jsonl`. View with `bun tools/usage.ts`. Pricing table covers Gemini, MiniMax, Claude.
...
**How to apply:** Use `npm run backup:status` to check sync health. Use `bun tools/usage.ts` to monitor API costs. Trust vault knowledge over auto-memory for production rules.
